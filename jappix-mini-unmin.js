...
XmlHttp.create = function() {
    try {
        if (typeof jXHR == "function") {
            if (window.XMLHttpRequest) {
                var req = new XMLHttpRequest();
                if (req.withCredentials !== undefined) return req;
            }
            return new jXHR();
        }
        if (window.XMLHttpRequest) {
...
Date.jab2date = function(ts) {
    ...
    if (ts.substr(ts.length - 6, 1) != 'Z') {
        ...
        if (ts.substr(ts.length - 6, 1) == '+') date.setTime(date.getTime() + offset.getTime() + date_offset);
        else if (ts.substr(ts.length - 6, 1) == '-') date.setTime(date.getTime() - offset.getTime() + date_offset);
    }
    return date;
};
...
JSJAC_CHECKQUEUEINTERVAL = 100;
JSJAC_CHECKINQUEUEINTERVAL = 100;
JSJAC_TIMERVAL = 2000;
JSJACHBC_MAX_HOLD = 1;
JSJACHBC_MAX_WAIT = 20;
JSJACHBC_BOSH_VERSION = "1.6";
JSJACHBC_USE_BOSH_VER = true;
JSJACHBC_MAXPAUSE = 20;
        ...
        if (arguments[2])
            if (JSJaCBuilder._isStringOrNumber(arguments[2]) || (arguments[2] instanceof Array)) {
                ...
                for (attr in arguments[2]) {
                    if (arguments[2].hasOwnProperty(attr) && attr != 'xmlns') element.setAttribute(attr, arguments[2][attr]);
                }
            } else element = this._createElement(doc, elementName, ns); if (arguments[3]) JSJaCBuilder._children(doc, element, arguments[3], ns);
        return element;
    },
...
JSJaCPacket.prototype.setXMLLang = function(xmllang) {
    if (BrowserDetect && (BrowserDetect.browser == 'Explorer') && (BrowserDetect.version >= 9)) return this;
...
JSJaCIQ.prototype.reply = function(payload) {
    ...
    rIQ.setType('result');
    if (payload) {
        if (typeof payload == 'string') rIQ.getChild().appendChild(rIQ.getDoc().loadXML(payload));
...
function JSJaCError(code, type, condition) {
    ...
    if (condition) xmldoc.documentElement.appendChild(xmldoc.createElement(condition)).setAttribute('xmlns', 'urn:ietf:params:xml:ns:xmpp-stanzas');
...
function JSJaCConnection(oArg) {
    ...
    else this.setPollInterval(JSJAC_TIMERVAL); if (oArg && oArg.httpbase) this._httpbase = oArg.httpbase;
    if (oArg && oArg.allow_plain) this.allow_plain = oArg.allow_plain;
    else this.allow_plain = JSJAC_ALLOW_PLAIN; if (oArg && oArg.cookie_prefix) this._cookie_prefix = oArg.cookie_prefix;
    else this._cookie_prefix = "";
...
JSJaCConnection.prototype.connect = function(oArg) {
    ...
    if (oArg.xmllang && oArg.xmllang != '') this._xmllang = oArg.xmllang;
    else this._xmllang = 'en';
    this.host = oArg.host || this.domain;
    this.port = oArg.port || 5222;
    if (oArg.secure) this.secure = 'true';
    else this.secure = 'false'; if (oArg.wait) this._wait = oArg.wait;
...
JSJaCConnection.prototype.registerHandler = function(event) {
    ...
    if (arguments.length > 2) eArg.childName = arguments[1];
    if (arguments.length > 3) eArg.childNS = arguments[2];
    ...
    for (var i = 0; i < this._events[event].length; i++) {
        ...
        if (typeof aEvent.handler == 'function') {
            try {
                if (arg) {
                    if (arg.pType) {
                        if ((!arg.getNode().hasChildNodes() && aEvent.childName != '*') || (arg.getNode().hasChildNodes() && !arg.getChild(aEvent.childName, aEvent.childNS))) continue;
                        if (aEvent.type != '*' && arg.getType() != aEvent.type) continue;
...
JSJaCConnection.prototype._parseStreamFeatures = function(doc) {
    ...
    if (doc.getElementsByTagNameNS) {
        ...
        for (var i = 0; i < errors.length; i++)
            if (errors.item(i).namespaceURI == "http://etherx.jabber.org/streams" || errors.item(i).getAttribute('xmlns') == "http://etherx.jabber.org/streams") {
...
JSJaCConnection.prototype._registerPID = function(pID, cb, arg) {
    ...
    if (arg) this._regIDs[pID].arg = arg;
...
JSJaCHttpBindingConnection.prototype.inherit = function(oArg) {
    ...
    if (oArg.wait) this._wait = oArg.wait;
...
JSJaCHttpBindingConnection.prototype._getRequestString = function(raw, last) {
    ...
    if (this._rid <= this._last_rid && typeof(this._last_requests[this._rid]) != 'undefined') reqstr = this._last_requests[this._rid].xml;
        ...
        if (last) reqstr += "type='terminate'";
        else if (this._reinit) {
            if (JSJACHBC_USE_BOSH_VER) reqstr += "xmpp:restart='true' xmlns:xmpp='urn:xmpp:xbosh' to='" + this.domain + "'";
            this._reinit = false;
        }
        if (xml != '' || raw != '') {
            reqstr += ">" + raw + xml + "</body>";
...
JSJaCHttpBindingConnection.prototype._getInitialRequestString = function() {
    ...
    if (JSJACHBC_USE_BOSH_VER) {
        ...
        if (this.authtype == 'sasl' || this.authtype == 'saslanon') reqstr += " xmpp:version='1.0'";
    }
    reqstr += "/>";
    return reqstr;
};
...
JSJaCHttpBindingConnection.prototype._handleInitialResponse = function(req) {
    ...
    if (body.getAttribute('polling')) this._min_polling = body.getAttribute('polling');
    if (body.getAttribute('inactivity')) this._inactivity = body.getAttribute('inactivity');
    if (body.getAttribute('requests')) this._setHold(body.getAttribute('requests') - 1);
    this.oDbg.log("set hold to " + this._getHold(), 2);
    if (body.getAttribute('ver')) this._bosh_version = body.getAttribute('ver');
    if (body.getAttribute('maxpause')) this._pause = Number.min(body.getAttribute('maxpause'), JSJACHBC_MAXPAUSE);
    this.setPollInterval(this._timerval);
    this._connected = true;
    this._inQto = setInterval(JSJaC.bind(this._checkInQ, this), JSJAC_CHECKINQUEUEINTERVAL);
    this._interval = setInterval(JSJaC.bind(this._checkQueue, this), JSJAC_CHECKQUEUEINTERVAL);
...
JSJaCHttpBindingConnection.prototype._parseResponse = function(req) {
    ...
    if (typeof(req.rid) != 'undefined' && this._last_requests[req.rid]) {
        if (this._last_requests[req.rid].handled) {
            ...
            return null;
        } else this._last_requests[req.rid].handled = true;
    }
    if (body.getAttribute("type") == "terminate") {
        ...
        if (condition != "item-not-found") {
            ...
            if (condition == "remote-stream-error")
                if (body.getElementsByTagName("conflict").length > 0) this._setStatus("session-terminate-conflict");
            if (condition == null) condition = 'session-terminate';
            ...
            this._errcnt++;
            if (this._errcnt > JSJAC_ERR_COUNT) this._abort();
        }
        return null;
    }
    this._errcnt = 0;
...
JSJaCHttpBindingConnection.prototype._reInitStreamWait = function(req, cb) {
    ...
    if (doc.getElementsByTagNameNS) {
        ...
        var features = doc.getElementsByTagNameNS('http://etherx.jabber.org/streams', 'features').item(0);
        if (features) {
            var bind = features.getElementsByTagNameNS('urn:ietf:params:xml:ns:xmpp-bind', 'bind').item(0);
        }
    } else {
        var featuresNL = doc.getElementsByTagName('stream:features');
        for (var i = 0, l = featuresNL.length; i < l; i++) {
            if (featuresNL.item(i).namespaceURI == 'http://etherx.jabber.org/streams' || featuresNL.item(i).getAttribute('xmlns') == 'http://etherx.jabber.org/streams') {
        ...
        if (features) {
            ...
            for (var i = 0, l = bind.length; i < l; i++) {
                if (bind.item(i).namespaceURI == 'urn:ietf:params:xml:ns:xmpp-bind' || bind.item(i).getAttribute('xmlns') == 'urn:ietf:params:xml:ns:xmpp-bind') {
                    bind = bind.item(i);
                    break;
                }
            }
...
var XML_LANG = 'en';
var JAPPIX_STATIC = 'http://grompe.github.io/jappix/';
var JAPPIX_VERSION = 'Stelo [0.7-gpe]';
var DEVELOPER = 'off';
var HOST_MUC = 'chat.grompe.org.ru';
var BOSH_HOST = 'http://chat.grompe.org.ru/http-bind';
...
function exists(selector) {
    if (jQuery(selector).size() > 0) return true;
    else return false;
}

function isConnected() {
    if ((typeof con != 'undefined') && con && con.connected()) return true;
    return false;
}
...
function explodeThis(toEx, toStr, i) {
    ...
    if (index != -1) {
        if (i == 0) toStr = toStr.substr(0, index);
        else toStr = toStr.substr(index + 1);
    }
    return toStr;
}
...
function thisResource(aXID) {
    if (aXID.indexOf('/') != -1) return explodeThis('/', aXID, 1);
    return '';
}
...
function stringPrep(string) {
    ...
    for (i in invalid) string = string.replace(invalid[i], valid[i]);
    return string;
}
...
function isDeveloper() {
    if (DEVELOPER == 'on') return true;
    return false;
}

function getXID() {
    if (con.username && con.domain) return con.username + '@' + con.domain;
    return '';
}

function generateColor(xid) {
    var colors = new Array('ac0000', 'a66200', '007703', '00705f', '00236b', '4e005c');
    var number = 0;
...
function logThis(data, level) {
    if (!isDeveloper() || (typeof(console) == 'undefined')) return false;
    switch (level) {
        case 0:
            console.debug(data);
            break;
...
function padZero(i) {
    if (i > -10 && i < 0) return '-0' + (i * -1);
    if (i < 10 && i >= 0) return '0' + i;
    return i;
}
...
function getXMPPTime() {
    var jInit = new Date();
    var jDate = jInit.getUTCFullYear() + '-';
    jDate += padZero(jInit.getUTCMonth() + 1) + '-';
    jDate += padZero(jInit.getUTCDate()) + 'T';
    jDate += padZero(jInit.getUTCHours()) + ':';
    jDate += padZero(jInit.getUTCMinutes()) + ':';
    jDate += padZero(jInit.getUTCSeconds()) + 'Z';
    return jDate;
}
...
function relativeDate(to_parse) {
    var current_date = Date.jab2date(getXMPPTime());
    ...
    if (isNaN(old_stamp) || isNaN(days)) return getCompleteTime();
    if (days <= 0) return old_time;
    if (days == 1) return _e("Yesterday") + ' - ' + old_time;
    if (days <= 7) return printf(_e("%s days ago"), days) + ' - ' + old_time;
...
function readMessageDelay(node) {
    ...
    if (d_delay) delay = d_delay;
    else {
        var x_delay = jQuery(node).find('x[xmlns="' + NS_DELAY + '"]:first').attr('stamp');
        if (x_delay) delay = x_delay.replace(/^(\w{4})(\w{2})(\w{2})T(\w{2}):(\w{2}):(\w{2})Z?(\S+)?/, '$1-$2-$3T$4:$5:$6Z$7');
    }
    return delay;
}
...
function applyLinks(string, mode, style) {
    var style, target;
    if (!style) style = '';
    else style = ' style="' + style + '"'; if (mode != 'xhtml-im') target = ' target="_blank"';
    else target = '';
...
function connectMini(domain, user, password) {
    try {
        oArgs = new Object();
        oArgs.httpbase = BOSH_HOST;
        con = new JSJaCHttpBindingConnection(oArgs);
        setupConMini(con);
        oArgs = new Object();
        oArgs.secure = true;
        oArgs.xmllang = XML_LANG;
        oArgs.resource = typeof MINI_RESOURCE == "undefined" ? 'jm' + Math.floor(Math.random() * 1000000) : MINI_RESOURCE;
        oArgs.domain = domain;
        if (MINI_ANONYMOUS) {
            oArgs.authtype = 'saslanon';
        } else {
            if (!MINI_NICKNAME) MINI_NICKNAME = user;
            MINI_NICKNAME_NOW = MINI_NICKNAME;
            oArgs.username = user;
            oArgs.pass = password;
        }
        con.connect(oArgs);
...
function connectedMini() {
    ...
    if (MINI_ANONYMOUS) initializeMini();
    else getRosterMini(); if (MINI_RECONNECT) logThis('Jappix Mini is now reconnected.', 3);
    else logThis('Jappix Mini is now connected.', 3);
    MINI_RECONNECT = 0;
}
...
function disconnectedMini() {
    ...
    if (!MINI_DISCONNECT || MINI_INITIALIZED) {
        ...
        if (MINI_INITIALIZED && (MINI_RECONNECT < 5)) {
            ...
            if (MINI_RECONNECT) reconnect_interval = (5 + (5 * MINI_RECONNECT)) * 1000;
            MINI_RECONNECT++;
            jQuery('#jappix_mini').oneTime(reconnect_interval, function() {
                launchMini(true, MINI_SHOWPANE, MINI_DOMAIN, MINI_USER, MINI_PASSWORD);
            });
        }
    } else launchMini(false, MINI_SHOWPANE, MINI_DOMAIN, MINI_USER, MINI_PASSWORD);
...
function handleMessageMini(msg) {
    ...
    if ((type == 'chat') || (type == 'normal') || (type == 'groupchat') || !type) {
        ...
        if (body) {
            ...
            if (type == 'groupchat') {
                if (msg.getChild('delay', NS_URN_DELAY) || msg.getChild('x', NS_DELAY)) message_type = 'old-message';
                if (!nick || subject) {
                    nick = '';
            ...
            if (!exists(target) && (type != 'groupchat')) chatMini(type, xid, nick, hash);
            displayMessageMini(type, body, use_xid, nick, hash, time, stamp, message_type);
            if ((!jQuery(target + ' a.jm_chat-tab').hasClass('jm_clicked') || !isFocused()) && (message_type == 'user-message')) notifyMessageMini(hash);
...
function handleIQMini(iq) {
    ...
    if ((iqQueryXMLNS == NS_VERSION) && (iqType == 'get')) {
        ...
        iqQuery.appendChild(iq.buildNode('name', {
            'xmlns': NS_VERSION
        }, 'Jappix Mini'));
        iqQuery.appendChild(iq.buildNode('version', {
            'xmlns': NS_VERSION
        }, JAPPIX_VERSION));
        iqQuery.appendChild(iq.buildNode('os', {
            'xmlns': NS_VERSION
        }, BrowserDetect.OS));
        con.send(iqResponse);
        ...
        var fArray = new Array(NS_DISCO_INFO, NS_VERSION, NS_ROSTER, NS_MUC, NS_VERSION, NS_URN_TIME);
        for (i in fArray)
            iqQuery.appendChild(iq.buildNode('feature', {
                'var': fArray[i],
                'xmlns': NS_DISCO_INFO
            }));
        con.send(iqResponse);
        ...
        iqTime.appendChild(iq.buildNode('tzo', {
            'xmlns': NS_URN_TIME
        }, getDateTZO()));
        iqTime.appendChild(iq.buildNode('utc', {
            'xmlns': NS_URN_TIME
        }, getXMPPTime()));
        con.send(iqResponse);
...
function handlePresenceMini(pr) {
    ...
    var type = pr.getType();
    var show = pr.getShow();
    if ((type == 'error') || (type == 'unavailable')) show = 'unavailable';
    else {
        switch (show) {
            case 'chat':
            case 'away':
    ...
    if (exists(groupchat_path)) {
        if (resource != unescape(jQuery(groupchat_path).attr('data-nick'))) {
            ...
            xid = from;
            hash = hex_md5(xid);
            if (show == 'unavailable') removeBuddyMini(hash, groupchat);
            else addBuddyMini(xid, hash, resource, groupchat);
        }
    }
    ...
    if (show == 'unavailable') {
        ...
        var extreason = 'Unavailable';
        var status = pr.getChild('status');
        if (status) {
            var code = status.getAttribute('code');
            var reason = pr.getChildVal('reason');
            if (code == '301') {
                extreason = 'Banned: ' + reason;
                if (resource == MINI_NICKNAME_NOW) {
                    alert('You have been banned from the room.\nReason: ' + reason);
                }
            } else if (code == '307') {
                extreason = 'Kicked: ' + reason;
                if (resource == MINI_NICKNAME_NOW) {
                    alert('You have been kicked from the room.\nReason: ' + reason);
                }
            }
        }
        jQuery(send_input).attr('disabled', true).attr('data-value', _e("Unavailable")).val(extreason);
    } else {
        jQuery(friend).attr('title', pr.getChildVal('status'));
...
function handleMUCMini(pr) {
    ...
    var resource = thisResource(from);
    var valid = false;
    if (!resource || (resource == unescape(jQuery('#jappix_mini #chat-' + hash + '[data-type=groupchat]').attr('data-nick')))) valid = true;
    if (valid && jQuery(xml).find('error[type="auth"] not-authorized').size()) {
        ...
        jQuery('#jappix_popup div.jm_prompt form').submit(function() {
            try {
                ...
                if (password) {
                    presenceMini('', '', '', '', from, password, false, handleMUCMini);
        ...
        presenceMini('', '', '', '', room + '/' + nickname, '', false, handleMUCMini);
        jQuery('#jappix_mini #chat-' + hash).attr('data-nick', escape(nickname));
        MINI_NICKNAME_NOW = nickname;
    } else handlePresenceMini(pr);
}

function presenceMini(type, show, priority, status, to, password, limit_history, handler) {
    var pr = new JSJaCPresence();
    if (to) pr.setTo(to);
    if (type) pr.setType(type);
    if (show) pr.setShow(show);
    if (priority) pr.setPriority(priority);
    if (status) pr.setStatus(status);
    if (password || limit_history) {
        var x = pr.appendNode('x', {
            'xmlns': NS_MUC
        });
        if (password) x.appendChild(pr.buildNode('password', {
            'xmlns': NS_MUC
        }, password));
        if (limit_history) x.appendChild(pr.buildNode('history', {
            'maxstanzas': 10,
            'seconds': 86400,
            'xmlns': NS_MUC
        }));
    }
    if (handler) con.send(pr, handler);
    else con.send(pr); if (!type) type = 'available';
    logThis('Presence sent: ' + type, 3);
}

function processCommandMini(aForm, body) {
    var xid = aForm.xid.value;
    var hash = hex_md5(xid);
    var myxid = bareXID(xid) + '/' + MINI_NICKNAME;
    var m;
    if (m = body.match(/^\/brb ?(.*)$/)) {
        var status = m[1] ? m[1] : 'brb';
        presenceMini('', 'away', '', status, myxid);
        return true;
    }
    if (m = body.match(/^\/away ?(.*)$/)) {
        var status = m[1] ? m[1] : 'away';
        presenceMini('', 'away', '', status, myxid);
        return true;
    }
    if (m = body.match(/^\/back ?(.*)$/)) {
        var status = m[1] ? m[1] : '';
        presenceMini('', '', '', status, myxid);
        return true;
    }
    ...
    if (m = body.match(/^\/(quit)|(exit)$/)) {
        disconnectMini();
        return true;
    }
    if (m = body.match(/^\/join (.+)$/)) {
        var chat_room = bareXID(generateXID(m[1]));
        chatMini('groupchat', chat_room, getXIDNick(chat_room), hex_md5(chat_room));
        return true;
    }
    if (m = body.match(/^\/clear$/)) {
        jQuery('#jappix_mini #chat-' + hash + ' div.jm_received-messages').empty();
        return true;
    }
    if (m = body.match(/^\/help$/)) {
...
function sendMessageMini(aForm) {
    try {
        ...
        if (body && xid) {
            aForm.body.value = '';
            if (processCommandMini(aForm, body)) return false;
            var aMsg = new JSJaCMessage();
            aMsg.setTo(xid);
            aMsg.setType(type);
            aMsg.setBody(body);
            con.send(aMsg);
            if (type != 'groupchat') displayMessageMini(type, body, getXID(), 'me', hash, getCompleteTime(), getTimeStamp(), 'user-message');
...
function smileyMini(image, text) {
    return ' <img class="jm_smiley jm_smiley-' + image + ' jm_images" alt="' + encodeQuotes(text) + '" src="' + JAPPIX_STATIC + 'blank.gif" /> ';
}
...
function notifyMessageMini(hash) {
    ...
    if (!exists(notify)) jQuery(tab).append('<span class="jm_notify">' + '<span class="jm_notify_left jm_images"></span>' + '<span class="jm_notify_middle">0</span>' + '<span class="jm_notify_right jm_images"></span>' + '</span>');
...
function notifyErrorMini() {
    jQuery('#jappix_mini').html('<div class="jm_starter">' + '<a class="jm_pane jm_button jm_images" href="https://mini.jappix.com/issues" target="_blank" title="' + _e("Click here to solve the error") + '">' + '<span class="jm_counter jm_error jm_images">' + _e("Error") + '</span>' + '</a>' + '</div>');
}

function notifyTitleMini() {
    if (MINI_TITLE == null) return false;
...
function insertNick(el, hash) {
    var nick = el.innerHTML;
    var input = jQuery('#jappix_mini #chat-' + hash + ' input.jm_send-messages');
    var text = input.attr('value');
    if (text == '') {
        input.attr('value', nick + ': ');
    } else {
        input.attr('value', text + nick + ' ');
    }
}
...
function createMini(domain, user, password) {
    ...
    jQuery('#jappix_mini a.jm_button').click(function() {
        try {
            ...
            if (jQuery(counter).text() == _e("Please wait...")) return false;
        ...
        try {
            ...
            jQuery('#jappix_popup div.jm_prompt form').submit(function() {
                try {
                    ...
                    if (join_this) {
                        chat_room = bareXID(generateXID(join_this));
                        chatMini('groupchat', chat_room, getXIDNick(chat_room), hex_md5(chat_room));
                    }
                } catch (e) {} finally {
                    return false;
    ...
    if (suspended) {
        ...
        notifyTitleMini();
    } else if (MINI_AUTOCONNECT) connectMini(domain, user, password);
    else {
        jQuery('#jappix_mini a.jm_pane.jm_button span.jm_counter').text(_e("Chat"));
        if (MINI_ANIMATE) {
            jQuery('#jappix_mini div.jm_starter').prepend('<span class="jm_animate jm_images_animate"></span>');
            ...
            jQuery('#jappix_mini div.jm_starter span.jm_animate').everyTime(10, function() {
                ...
                jQuery(this).css('margin-top', m_top + 'px').css('margin-left', m_left + 'px');
            });
        }
    }
}
...
function displayMessageMini(type, body, xid, nick, hash, time, stamp, message_type) {
    ...
    var grouped = false;
    var header = '';
    if ((last_xid == xid) && (message_type == last_type) && ((stamp - last_stamp) <= 1800)) grouped = true;
    else {
        if (nick) header += '<span class="jm_date">' + time + '</span>';
        if (type == 'groupchat') header += '<b style="color: ' + generateColor(nick) + ';" data-xid="' + encodeQuotes(xid) + '" onclick="insertNick(this, \'' + hash + '\')">' + nick.htmlEnc() + '</b>';
        else if (nick == 'me') header += '<b class="jm_me" data-xid="' + encodeQuotes(xid) + '">' + _e("You") + '</b>';
    ...
    var message_addstyle = '';
    if (body.match(MINI_NICKNAME_NOW)) {
        message_addstyle = ' jm_highlight';
    }
    body = body.htmlEnc();
    body = applyLinks(body, 'mini');
    if (me_command) body = '<em>' + body + '</em>';
    body = '<p>' + body + '</p>';
    if (grouped) jQuery('#jappix_mini #chat-' + hash + ' div.jm_received-messages div.jm_group:last').append(body);
    else jQuery('#jappix_mini #chat-' + hash + ' div.jm_received-messages').append('<div class="jm_group jm_' + message_type + message_addstyle + '" data-type="' + message_type + '" data-stamp="' + stamp + '">' + header + body + '</div>'); if (can_scroll) messageScrollMini(hash);
}
...
function openPromptMini(text, value) {
    ...
    closePromptMini();
    jQuery('body').append('<div id="jappix_popup">' + '<div class="jm_prompt">' + '<form>' + text + '<input class="jm_text" type="text" value="" />' + '<input class="jm_submit" type="submit" value="' + _e("Submit") + '" />' + '<input class="jm_submit" type="reset" value="' + _e("Cancel") + '" />' + '<div class="jm_clear"></div>' + '</form>' + '</div>' + '</div>');
    var vert_pos = '-' + ((jQuery(prompt).height() / 2) + 10) + 'px';
    jQuery(prompt).css('margin-top', vert_pos);
    if (value) jQuery(value_input).val(value);
    jQuery(document).oneTime(10, function() {
...
function chatMini(type, xid, nick, hash, pwd, show_pane) {
    ...
    if (!exists(current)) {
            ...
            if (!nickname) {
                ...
                jQuery('#jappix_popup div.jm_prompt form').submit(function() {
                    try {
                        ...
                        if (nickname) MINI_NICKNAME = nickname;
        ...
        if (type == 'groupchat') {
            ...
            presenceMini('', '', '', '', xid + '/' + nickname, pwd, false, handleMUCMini);
        }
    }
...
function initializeMini() {
    MINI_INITIALIZED = true;
    if (!MINI_ANONYMOUS) presenceMini();
    for (var i = 0; i < MINI_GROUPCHATS.length; i++) {
        if (!MINI_GROUPCHATS[i]) continue;
        try {
            var chat_room = bareXID(generateXID(MINI_GROUPCHATS[i]));
            chatMini('groupchat', chat_room, getXIDNick(chat_room), hex_md5(chat_room), MINI_PASSWORDS[i], MINI_SHOWPANE);
        } catch (e) {}
    }
    if (!MINI_AUTOCONNECT && !MINI_GROUPCHATS.length) jQuery(document).oneTime(10, function() {
        showRosterMini();
    });
}
...
function addBuddyMini(xid, hash, nick, groupchat) {
    ...
    if (groupchat) {
        ...
        if (!exists(path)) {
            jQuery('#jappix_mini div.jm_roster div.jm_buddies').append('<div class="jm_grouped" data-xid="' + escape(groupchat) + '">' + '<div class="jm_name">' + getXIDNick(groupchat).htmlEnc() + '</div>' + '</div>');
        }
    }
    ...
    if (groupchat) jQuery(path).append(code);
    else jQuery(path).prepend(code);
...
function removeBuddyMini(hash, groupchat) {
    ...
    if (groupchat && !jQuery(group + ' a.jm_friend').size()) jQuery(group).remove();
    return true;
}
...
function handleRosterMini(iq) {
    jQuery(iq.getQuery()).find('item').each(function() {
        ...
        if (!isGateway(xid)) {
            ...
            var hash = hex_md5(xid);
            if (!nick) nick = getXIDNick(xid);
            if (subscription == 'remove') removeBuddyMini(hash);
            else addBuddyMini(xid, hash, nick);
        }
    });
    if (!MINI_INITIALIZED) initializeMini();
...
function launchMini(autoconnect, show_pane, domain, user, password) {
    ...
    if (!user || !password) MINI_ANONYMOUS = true;
    else MINI_ANONYMOUS = false; if (autoconnect && hasDB()) MINI_AUTOCONNECT = true;
    else MINI_AUTOCONNECT = false; if (show_pane) MINI_SHOWPANE = true;
    else MINI_SHOWPANE = false;
    jQuery('#jappix_mini').remove();
    if (MINI_RECONNECT) {
        logThis('Trying to reconnect (try: ' + MINI_RECONNECT + ')!');
        return createMini(domain, user, password);
    }
    jQuery('head').append('<link rel="stylesheet" href="' + JAPPIX_STATIC + 'mini.css" type="text/css" media="all" />');
    if ((BrowserDetect.browser == 'Explorer') && (BrowserDetect.version < 7)) jQuery('head').append('<link rel="stylesheet" href="' + JAPPIX_STATIC + 'mini-ie.css" type="text/css" media="all" />');
    jQuery(document).keydown(function(e) {
        if ((e.keyCode == 27) && !isDeveloper()) return false;
    });
